...
def sample_gt(gt, train_size, mode='random'):
    ...
    if mode == 'random':
        if train_size == 0:
            ...
            print('Check the percentage of the training examples used',train_gt.shape,np.mean(train_gt!=0))
            #In https://github.com/nshaud/DeepHyperX/blob/master/utils.py
            #Lines 465 and 466, unfortunately uses a lot more training examples than allowed by the “training_sample” say we use 10% of the training data but then it uses a lot more (nearly 50% of the pixels). 
            #
            #These were the original problematic lines:
            #    train_gt[train_indices] = gt[train_indices]
            #    test_gt[test_indices] = gt[test_indices]
            #
            #You could see the problem simply by checking the percentage of nonzero entries we have after these two lines by:
            #print(np.mean(train_gt!=0))
