...
purposes only. For commercial use, please contact the authors.
"""
...
parser.add_argument("--dataset", 
    type=str, default="IndianPines", choices=dataset_names, help="Dataset to use."
)
...
for run in range(N_RUNS):
    ...
    if MODEL == "SVM_grid":
    ...
    elif MODEL == "randomForest_TR_TS":
        from sklearn.ensemble import RandomForestClassifier as Classifier
        
        data_version = int(DATASET[-2:])
        trts = data_version % 2

        if CHECKPOINT is not None or trts == 0:  #1 means training run, 0 means test run (using the previous number's checkpoint)
            if CHECKPOINT is None:
                DATASET_TR = DATASET[:-2] + str(data_version-1).zfill(2)
                clf = load_model(MODEL, DATASET_TR)
            else:
                clf = load_model(MODEL, DATASET_TR, filename = CHECKPOINT)

        else:  #since it is a training run then we should create a checkpoint (a trained classifier)
            X_train, y_train = build_dataset(img, gt, ignored_labels=IGNORED_LABELS)
            X_train, y_train = sklearn.utils.shuffle(X_train, y_train)
            class_weight = "balanced" if CLASS_BALANCING else None
            clf = Classifier(class_weight=class_weight)
            clf.fit(X_train, y_train)    #give the whole dataset for training and saving the model for later use on another image
            save_model(clf, MODEL, DATASET)
            print('Trained on the whole image and saved model.... now trying train-test split for training stats...')
            X_train, y_train = build_dataset(img, train_gt, ignored_labels=IGNORED_LABELS)
            X_train, y_train = sklearn.utils.shuffle(X_train, y_train)
            class_weight = "balanced" if CLASS_BALANCING else None
            clf.fit(X_train, y_train) #give a small sample for training
            
        prediction = clf.predict(img.reshape(-1, N_BANDS)) #give the whole dataset for testing
        prediction = prediction.reshape(img.shape[:2])
        ...
        if CHECKPOINT is not None or trts == 0:
            if CHECKPOINT is None:
                DATASET_TR = DATASET[:-2] + str(data_version-1).zfill(2)
                modeldict = load_model(MODEL, DATASET_TR, filename = 'NN_TYPE')
            ...
            save_model(model, MODEL, DATASET, epoch=None)
